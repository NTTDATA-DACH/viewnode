...
description: Execute the 4+1 architecture workflow and generate architecture view
  artifacts.
compatibility: Requires spec-kit project structure with .specify/ directory
metadata:
  author: github-spec-kit
  source: arch:commands/speckit.arch.generate.md
---
...
1. **Setup**: Run `.specify/extensions/arch/scripts/bash/setup-arch.sh --json` from repo root and parse JSON for `ARCH_FILE`, `ARCH_DIR`, `SCENARIO_VIEW`, `LOGICAL_VIEW`, `PROCESS_VIEW`, `DEVELOPMENT_VIEW`, and `PHYSICAL_VIEW`. The setup JSON may also include `REPO_FACTS_FILE` for reverse workflow compatibility; acknowledge it and ignore it for this generate workflow.
